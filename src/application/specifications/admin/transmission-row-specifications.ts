...
export class TransmissionRowSpecification extends Specification<TransmissionRows> {
  constructor(
  ...
  isSatisfiedBy(entity: TransmissionRows): boolean {
    const matchesRole = entity.network?.id === this.networkId;

    return matchesRole;
  }

  toWhereClause(): Partial<Record<keyof TransmissionRows, any>> {
    const whereClause: Partial<Record<keyof TransmissionRows, any>> = {};
    if (this.networkId) {
